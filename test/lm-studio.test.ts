...
void test(
  "LM Studio Integration Suite",
  {
    skip:
      process.platform === "win32"
        ? "Skipped on Windows CI due to Node 24 undici connection pooling teardown behavior"
        : false,
  },
  async (t) => {
    await t.test(
      "sends authenticated non-streaming JSON Schema requests and validates output",
      async (t) => {
    ...
    await t.test(
      "omits Authorization when optional authentication is not configured",
      async (t) => {
        const captured: CapturedRequest[] = [];
        const baseUrl = await startFakeServer(t, async (request, response) => {
          captured.push(await capture(request));
        ...
        assert.deepEqual(
          (await client.inferStructured(inferenceRequest())).output,
          {
            ok: true,
          },
        );
    ...
    await t.test(
      "rejects unauthorized and unavailable models without fallback",
      async (t) => {
        let requestCount = 0;
    ...
    await t.test(
      "detects enforced authentication without exposing either token",
      async (t) => {
        const baseUrl = await startFakeServer(t, (request, response) => {
    ...
    await t.test(
      "detects when LM Studio accepts a deliberately invalid token",
      async (t) => {
    ...
    await t.test(
      "retries one transient inference failure and never retries a permanent failure",
      async (t) => {
        let chatRequests = 0;
        const baseUrl = await startFakeServer(t, (request, response) => {
          if (request.url === "/v1/models") {
        ...
        chatRequests = 0;
        const permanentBaseUrl = await startFakeServer(
          t,
          (request, response) => {
            if (request.url === "/v1/models") {
              json(response, 200, { data: [{ id: MODEL }] });
        ...
        await assert.rejects(
          configuredClient(permanentBaseUrl).inferStructured(
            inferenceRequest(),
          ),
    ...
    await t.test(
      "returns a structured transient error after the bounded retry",
      async (t) => {
        let chatRequests = 0;
        const baseUrl = await startFakeServer(t, (request, response) => {
          if (request.url === "/v1/models") {
    ...
    await t.test(
      "cancellation and deadlines abort the HTTP stack without retry",
      async (t) => {
        let chatRequests = 0;
        const baseUrl = await startFakeServer(t, (request, response) => {
          if (request.url === "/v1/models") {
    ...
    await t.test(
      "fails closed on malformed, oversized, and partial responses",
      async (t) => {
        const cases = [
          {
            name: "malformed",
    ...
    await t.test(
      "embedText: batch input returns one embedding per input string",
      async (t) => {
        const baseUrl = await startFakeServer(t, (request, response) => {
          if (request.url === "/v1/models") {
            json(response, 200, { data: [{ id: EMBEDDING_MODEL }] });
    ...
    await t.test("embedText: rejects unauthorized model", async (t) => {
      const baseUrl = await startFakeServer(t, (_request, response) => {
        json(response, 200, { data: [{ id: EMBEDDING_MODEL }] });
      });
      const client = createLmStudioClient({
    ...
    await t.test("embedText: rejects unavailable model", async (t) => {
      const baseUrl = await startFakeServer(t, (request, response) => {
        if (request.url === "/v1/models") {
          json(response, 200, { data: [{ id: "different/model" }] });
          return;
    ...
    await t.test(
      "embedText: malformed response returns malformed_response",
      async (t) => {
        const baseUrl = await startFakeServer(t, (request, response) => {
          if (request.url === "/v1/models") {
            json(response, 200, { data: [{ id: EMBEDDING_MODEL }] });
    ...
    await t.test(
      "embedText: retries one transient 503 and succeeds",
      async (t) => {
    ...
    await t.test("embedText: timeout aborts the request", async (t) => {
      const baseUrl = await startFakeServer(t, (request, response) => {
        if (request.url === "/v1/models") {
          json(response, 200, { data: [{ id: EMBEDDING_MODEL }] });
          return;
    ...
    await t.test(
      "embedText: cancellation via AbortSignal propagates",
      async (t) => {
        const baseUrl = await startFakeServer(t, (request, response) => {
          if (request.url === "/v1/models") {
            json(response, 200, { data: [{ id: EMBEDDING_MODEL }] });
    ...
    await t.test("embedText: oversized response is rejected", async (t) => {
      const baseUrl = await startFakeServer(t, (request, response) => {
        if (request.url === "/v1/models") {
          json(response, 200, { data: [{ id: EMBEDDING_MODEL }] });
          return;
    ...
    await t.test(
      "embedText: omits Authorization when no token configured",
      async (t) => {
        const captured: CapturedRequest[] = [];
        const baseUrl = await startFakeServer(t, async (request, response) => {
          captured.push(await capture(request));
